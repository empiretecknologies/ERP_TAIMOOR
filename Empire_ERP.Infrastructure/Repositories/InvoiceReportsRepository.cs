    ...
    public class InvoiceReportsRepository : IInvoiceReportsRepository
    {
        ...
        public MyHttpResponseMessage GetReportData(InvoiceReports report, Common common)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    if (report.ReportID < 24 || report.ReportID == 100 || report.ReportID == 140 || report.ReportID ==144)
                    {
                        ...
                        else if (report.ReportID == 144)
                        {
                            while (reader.Read())
                            {
                                var row = new CustomInvoiceReports
                                {
                                    ...
                                    Amt = reader["TOTAL_AMT"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["TOTAL_AMT"]),
                                    MonthlyValues = new Dictionary<string, decimal>()
                                };
                                for (int i = 0; i < reader.FieldCount; i++)
                                {
                                    string columnName = reader.GetName(i);
                                    if (columnName != "ACT_NAME" && columnName != "CATEGORY" && columnName != "PARTY_NAME" && columnName != "TOTAL_AMT")
                                    {
                                        row.MonthlyValues[columnName] = reader[i] == DBNull.Value ? 0 : Convert.ToDecimal(reader[i]);
                                    }
                                }
                                jsonDataResult.Add(row); 
                            }
