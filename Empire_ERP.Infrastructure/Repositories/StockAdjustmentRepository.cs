        ...
        public MyHttpResponseMessage GetStockAdjustmentDetailByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $"SELECT T.TRAN_ID, T.DT_CODE, T.CHK, T.RATE,T.AMT, T.PICK_ID, T.ITEM_CODE, T.QTY, T.UNIT, T.QTY2, T.BAL_QTY, T.DT_DESC, T.COLOR, T.SIZE, T.GRADE, CASE WHEN '{b_i}' = 'B' THEN IB.ITEM_NAME ELSE IM.ITEM_NAME END AS ITEM_ID " +
                            $" FROM {table} T" +
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                DT_CODE = reader["DT_CODE"] == DBNull.Value ? string.Empty : Convert.ToString(reader["DT_CODE"]),
                                ITEM_CODE = reader["ITEM_CODE"] == DBNull.Value ? 0 : Convert.ToInt32(reader["ITEM_CODE"]),
                                QTY = reader["QTY"] == DBNull.Value ? "0" : Convert.ToString(reader["QTY"]), 
                                UNIT = reader["UNIT"] == DBNull.Value ? 0 : Convert.ToInt32(reader["UNIT"]),
                                QTY2 = reader["QTY2"] == DBNull.Value ? "0" : Convert.ToString(reader["QTY2"]),
                                BAL_QTY = reader["BAL_QTY"] == DBNull.Value ? "0" : Convert.ToString(reader["BAL_QTY"]),
                                DT_DESC = reader["DT_DESC"] == DBNull.Value ? string.Empty : Convert.ToString(reader["DT_DESC"]),
                                ...
                                CHK = reader["CHK"] == DBNull.Value ? string.Empty : Convert.ToString(reader["CHK"]),
                                PICK_ID = reader["PICK_ID"] == DBNull.Value ? string.Empty : Convert.ToString(reader["PICK_ID"]),
                                ITEM_ID = reader["ITEM_ID"] == DBNull.Value ? string.Empty : Convert.ToString(reader["ITEM_ID"]),
                                CHK1 = reader["CHK"] != DBNull.Value && Convert.ToString(reader["CHK"]) == "1",
                            };
                            jsonDataResult.Add(row);
                        }
                        reader.Close();
                    ...
                    if (isStockSufficient)
                    {
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            ...
                            try
                            {
                                ...
                                insertQuery.Append($"INSERT INTO {detailTable} (TRAN_ID, DT_CODE,AMT, RATE, ITEM_CODE, QTY, UNIT, QTY2, BAL_QTY, DT_DESC, COLOR, SIZE, GRADE, BCODE, PERIOD_ID, " +
            ...
            try
            {
                ...
                if (menuDetails.MD_ID == 68)
                {
                    ...
                    string query = $@"
                                ...
                                FROM TBL_STKA_MASTER M
                                LEFT JOIN TBL_STKA_DETAIL D 
                                    ON D.TRAN_ID = M.TRAN_ID
                                    AND D.BCODE = M.BCODE
                                LEFT JOIN TBL_ITEMSMASTER IM WITH (NOLOCK)
                                    ON IM.ITEM_CODE = D.ITEM_CODE
                                LEFT JOIN TBL_COLOR C WITH (NOLOCK)
                                    ON C.GROUP_CODE = D.COLOR
                                LEFT JOIN TBL_SIZE S WITH (NOLOCK)
                                    ON S.GROUP_CODE = D.SIZE
