$(document).keydown(function (e) {
    //Check Ctrl key is pressed and S key is pressed Saveempr_PurchaseOrder
...
var empr_PurchaseOrder = {
    ...
    InitEvents: function () {
        $(document).ready(function () {
            ...
            //empr_PurchaseOrder.InitItemIds();
            empr_PurchaseOrder.InitReportTypeDDL();

            var Id = 0;
            //empr_PurchaseOrder.InitSalesman(Id);
            ...
            $("#TERMS").on("input", function () {
                ...
                for (let i = 0; i < rowCount; i++) {
                    ...
                    let result = empr_PurchaseOrder.calculateDue(deliveryDate, payTerm);
            ...
            $('body').on('click', '#BtnSodaPick', function () {
                empr_PurchaseOrder.InitSodaPickGrid();
            });

            $('#COMM').on('input', function () {
                empr_PurchaseOrder.CalculateCommition();
            });
            ...
            $('body').on('click', '#BtnAddBarcodes', function () {
                ...
                if (selectedBarcodes.length > 0) {
                    empr_PurchaseOrder.AddBarcodeToGrid();
                }
                else {
            ...
            $('body').on('click', '#BtnAddSodaDetailToDelivery', function () {
                ...
                if (selectedSodas.length > 0) {
                    empr_PurchaseOrder.AddSodaToDelivery();
                }
                else {
                    empr_helper.notify("Please select the items first.", 2);
                }
    ...
    InitSodaPickGrid: function () {
        ...
        if (PARTY_CODE == "" || PARTY_CODE == null || PARTY_CODE == undefined || PARTY_CODE == 0) {
            ...
            empr_PurchaseOrder.GetPickDataByParty(PARTY_CODE, ACT_CODE);
        }

    },

    GetPickDataByParty: function (PARTY_CODE, ACT_CODE) {
        ajaxHelper.ajaxGetJson('/PurchaseOrder/GetPickDataByParty?partyCode=' + PARTY_CODE + '&actCode=' + ACT_CODE, function (data) {
            console.log('data', data);
            if (data.msgType == 1) {
                if (data.data.length > 0) {
                    if (typeForm == 'MPO') {
                        empr_PurchaseOrder.CreatePickDetailGrid(data.data);
                    ...
                    empr_helper.notify("Purchase Order not found.", 2);
                }
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
    ...
    SetData: function (dataSource) {
        $.each(dataSource, function (index, item) {
            ...
            item.dT_CODE = 0;
            item.__KEY__ = empr_PurchaseOrder.GenerateKey(36);
        });
    ...
    CreatePickDetailGrid: function (dataSrc) {
        ...
        var col = [];
        var updatedData = [];
        if (empr_PurchaseOrder.formName == 'MPO') {
            ...
            updatedData = dataSrc
                .map(item => {
                    ...
                    const totalPack = matchingRows.reduce((sum, row) => parseInt(sum) + parseInt(row.totaL_PACK), 0);
            ...
            empr_PurchaseOrder.originalValues = {};
            updatedData.forEach(row => {
                empr_PurchaseOrder.originalValues[row.picK_ID_D] = row.totaL_PACK;
            });
            ...
            col = [
                { dataField: 'lB_DATE', caption: 'Date', visible: true, allowEditing: false, },
                {
                    ...
                    cellTemplate: function (container, options) {
                        $('<a>')
                            ...
                            .text(options.value)
                            .attr('href', '#')
                            .attr('onclick', 'empr_PurchaseOrder.openVoucherPage(' + JSON.stringify(options.data.link) + ', ' + JSON.stringify(options.data.id) + ')')
                            .appendTo(container);
                    }
                },
                ...
                { dataField: 'iqty', caption: 'I.Qty', dataType: 'number', width: 90, allowEditing: false, },
                {
                    ...
                    setCellValue: function (newData, value, currentRowData) {
                        const originalQty = empr_PurchaseOrder.originalValues[currentRowData.picK_ID_D];
    ...
    CreateGrid: function (dataSource, dueCalc) {
        ...
        dataSource.forEach(item => {
            ...
            if (item.picK_ID_D > 0) {
                empr_PurchaseOrder.pickIds.push(item.picK_ID_D);
                $('#pickItems').hide();
            }
        });
        if (dataSource.length > 0) {
            empr_PurchaseOrder.rowsCount = dataSource.length - 1;
        }
        //var data_source;
        ...
        //        return empr_PurchaseOrder.ItemsOnParty;
        //    };
        //}
        ...
        var col = [
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    empr_PurchaseOrder.setPurchaseBillItemData(newData, value, currentRowData.qty);
                }
            },
            //{
            //    dataField: 'grade',
            ...
            //    setCellValue: function (newData, value, currentRowData) {
            //        newData.grade = value;
            //        var itemCode = parseFloat(currentRowData.iteM_CODE) || 0;

            //        if (Grades && Grades.length > 0) {

            //            var selectedItem = Grades.find(u => u.key === value);

            //            if (selectedItem) {
            //                var rate = parseFloat(selectedItem.rate) || 0;
            //                newData.size = parseFloat(selectedItem.size) || 0;

            //                if ((currentRowData.rate == 0 || currentRowData.rate == null || currentRowData.rate == undefined) && rate > 0) {
            //                    newData.rate = rate;
            //                }
            //                else if (currentRowData.rate > 0) {
            ...
            //},
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    //if (dcType == 'SB') {
                    //    var updatedItems = empr_PurchaseOrder.ItemsOnParty;
                    //}
                    //else {
                    //}
            ...
            //},
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    // dcType wali condition comment out ki gayi hai
                    //if (dcType == 'SB') {
        ...
        empr_helper.editableDxGridbindingForTransactionsVouchers('#DetailContainer', col, dataSource, "PurchaseOrder", "iteM_CODE");
        if (dataSource.length == 0) {
    ...
    //                    const copyAction = !Permissions.r_COPY
    //                        ? ''
    //                        : `<a href="javascript:;" class="grid-action-icon Clone" onclick="empr_PurchaseOrder.CommCloneRow(${options.rowIndex})" title="Duplicate"><i class="fa fa-clone"></i></a>`;
    //                    const addAction = (!Permissions.r_ADD && !Permissions.r_EDIT)
    //                        ? ''
    //                        : `<a href="javascript:;" class="grid-action-icon Add" style="margin-left: 8px" onclick="empr_PurchaseOrder.CommAddRow()" title="Add"><i class="fa fa-add"></i></a>`;
    //                    const deleteAction = !Permissions.r_DLT
    //                        ? ''
    //                        : `<a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_PurchaseOrder.CommDeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>`;
    //                    const actions = `<div class="btn-group btn-group-sm">${copyAction}${addAction}${deleteAction}</div>`;
    //                    $(actions).appendTo(container);
    //                }
    //                else {
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                //dataSource.unshift({ __KEY__: empr_PurchaseOrder.GenerateKey(36), dT_CODE: 0 });
                let payTerm = parseInt($('#TERMS').val(), 10) || 0;
                ...
                const newRow = {
                    ...
                    adv: empr_PurchaseOrder.PartysaleTax,
                    deL_DATE: todayDate,
                    warehouse: 2,
                    disc: empr_PurchaseOrder.PartyDisc
                };
        ...
        if (empr_PurchaseOrder.pickIds.length > 0) {
            ...
            if ($('#CommContainer').dxDataGrid('instance').hasEditData()) {
                $('#CommContainer').dxDataGrid('instance').saveEditData().done(function () {
                    ...
                    dataSource.unshift({ __KEY__: empr_PurchaseOrder.GenerateKey(36), dT_CODE: 0, traN_ID: empr_PurchaseOrder.CommissionTranId });
                    gridInstance.option("dataSource", dataSource);
                    gridInstance.refresh();
    ...
    //                buttonsStyling: false
    //            }).then(function () {
    //                ajaxHelper.ajaxPostJsonData({ code: dtCode }, "/PurchaseOrder/DeleteCommDetailByCode", function (data) {
    //                    empr_helper.notify(data.msg, data.msgType);
    //                    if (data.msgType == 1) {
    //                        gridInstance.deleteRow(index);
    //                        empr_PurchaseOrder.rowsCount -= 1;
    ...
    //        );
    //    });
    //    empr_PurchaseOrder.ItemsOnParty = resultItems;

    //},

    //ItemsBehalfOnParty: function (PARTY_CODE, ACT_CODE) {

    //    ajaxHelper.ajaxGetJson('/PurchaseOrder/ItemsBehalfOnParty?partyCode=' + PARTY_CODE + '&actCode=' + ACT_CODE, function (data) {
    //        if (data.length > 0) {
    //            empr_PurchaseOrder.ItemsOnParty = data;
    //        }
    //        else {
    //            empr_helper.notify(data.msg, data.msgType);
    //        }
    ...
    //            // Yahan pe grid instance ko access karte hain
    //            var grid = $('#DetailContainer').dxDataGrid('instance');

    //            // Lookup ke dataSource ko update karte hain
    ...
    //            // Agar aapko UI ko force refresh karna hai, toh aap yeh bhi kar sakte hain
    ...
    InitQuickSearchGrid: function () {
        ...
        //empr_PurchaseOrder.CreateQuickSearchGrid();
    },
        ...
        ajaxHelper.ajaxGetJson('/PurchaseOrder/GetPurchaseOrderByCode?code=' + code, function (data) {
            if (data.master.msgType == 1) {
                ...
                if (masterData.length == 1) {
                    ...
                    //empr_PurchaseOrder.ItemsBehalfOnParty(filteredData[0].partyCode, filteredData[0].accountCode);
                    ...
                    //empr_PurchaseOrder.InitSalesman(0, parseInt(response.scode));
                ...
                //        empr_PurchaseOrder.CommissionTranId = data.commission.data[0].traN_ID;
                //        $('.nav-item.commission').show();
                //        empr_PurchaseOrder.CreateCommGrid(data.commission.data);
                //    }
                //}
                //else {
                //    empr_helper.notify(data.msg, data.msgType);
                ...
                if (data.detail.msgType == 1) {
                    empr_PurchaseOrder.CreateGrid(data.detail.data, false);
                }
                else {
                    empr_helper.notify(data.msg, data.msgType);
                }
    ...
    //    ajaxHelper.ajaxGetJson('/PurchaseOrder/GetPurchaseBillDetailByItem?code=' + code + '&qty=' + qty, function (data) {
    //        if (data.msgType == 1) {
    //            if (data.data.length > 0) {
    //                empr_PurchaseOrder.rowsCount += data.data.length;
    //                const gridInstance = $('#DetailContainer').dxDataGrid('instance');
    //                var dataSource = gridInstance.option("dataSource");
    //                $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
    ...
    //    ajaxHelper.ajaxGetJson('/PurchaseOrder/GetPurchaseBillPickDetailByCode?code=' + code, function (data) {
    //        if (data.msgType == 1) {
    //            if (Type == "I") {
    //                empr_PurchaseOrder.CreateGrid(data.data);
    //            } else {
    ...
    ValidateMainInfo: function () {
        ...
        $.each(data.Detail, function (index, item) {
            ...
            if (item.amt == "" || item.amt == null || item.amt == undefined) {
                empr_helper.notify("Invalid Amount at line No " + (index + 1), 2);
                valid = false;
                return valid;
            }
    ...
    Save: function () {
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/PurchaseOrder/Save", function (data) {
            ...
            if (data.msgType == 1) {
                ...
                if (dataClear == 1) {
                    empr_PurchaseOrder.GetPurchaseBillByCode(data.data.code);
                    //empr_PurchaseOrder.GetPurchaseBillDetailByCode(data.data.code);
                    $('#BtnDelete').show();
                }
                else {
                    $("#SCODE").dxSelectBox("instance")?.option("value", "");
    ...
    ResetForm: function () {
        ...
        empr_PurchaseOrder.CreateGrid([{ __KEY__: empr_PurchaseOrder.GenerateKey(36), chK1: false, chk: "0", dT_CODE: 0, warehouse: 2, deL_DATE: todayDate }], true);
        //empr_PurchaseOrder.CreateCommGrid([{ __KEY__: empr_PurchaseOrder.GenerateKey(96), chK1: false, chk: "0", dT_CODE: 0 }]);
        ...
        empr_PurchaseOrder.pickIds = [];
        empr_PurchaseOrder.ItemsOnParty = [];
        empr_PurchaseOrder.firstClick = 0;
        empr_PurchaseOrder.PartysaleTax = 0;
        empr_PurchaseOrder.PartyDisc = 0;
        empr_PurchaseOrder.CommissionTranId = 0;
        empr_PurchaseOrder.isSalesman = false;
        //empr_PurchaseOrder.CreateGrid([]);
        empr_PurchaseOrder.InitPartyType();
        empr_PurchaseOrder.InitCurrencyDDL();
        //empr_PurchaseOrder.InitCommissionAmtDDL("PR");
    ...
    InitPartyType: function () {
        empr_PurchaseOrder.bindDxDdl("PARTY_CODE", PartyType, null, "key", "value", "Select", function (d) {
    ...
    OnPartyChange: function (d) {
        if (d.value == null || d.value == '') {
            ...
            empr_PurchaseOrder.InitSalesman(0);
        }
        else {
            var filteredData = $.grep(PartyType, function (item) {
            ...
            empr_PurchaseOrder.PartysaleTax = parseFloat(filteredData[0].sTax) || 0;
            empr_PurchaseOrder.PartyDisc = parseFloat(filteredData[0].disc) || 0;
            //empr_PurchaseOrder.SetSaleTax(filteredData[0].sTax, filteredData[0].disc);
            empr_PurchaseOrder.ItemsOnParty = [];
            //empr_PurchaseOrder.ItemsBehalfOnParty(filteredData[0].partyCode, filteredData[0].accountCode);
            //empr_PurchaseOrder.CreateGrid([{ __KEY__: empr_PurchaseOrder.GenerateKey(36), chK1: false, chk: "0", dT_CODE: 0, warehouse: 2, deL_DATE: todayDate }], true);
            var rinvDate = $('#RINV_DATE').val();

            let result = empr_PurchaseOrder.calculateDue(rinvDate, filteredData[0].paymentTerms);
            ...
            if (filteredData[0].partyCode != "" && filteredData[0].partyCode != 0) {
                empr_PurchaseOrder.InitSalesman(filteredData[0].partyCode, parseInt(filteredData[0].scode));
            }
        }
    },

    //InitItemIds: function () {
    //    empr_PurchaseOrder.bindDxDdl("PICK_ITEM", ItemIds, null, "key", "itemId", "Select", function (d) {
    //        if (d.value == null || d.value == '') {
    //            $('#itemIdHidden').val('');
    //        }
    //        else {
    ...
    InitSalesman: function (Id, selectedValue = null) {
        ajaxHelper.ajaxGetJson('/PurchaseOrder/GetSalesmanByParty?id=' + Id, function (data) {
            if (data.data.table.length > 0) {
                empr_PurchaseOrder.bindDxDdl("SCODE", data.data.table, selectedValue, "partY_CODE", "partY_NAME", "Select", function (selected) {
                ...
                ajaxHelper.ajaxGetJson('/PurchaseOrder/GetSalesmanByParty?id=' + 0, function (data) {
                    empr_PurchaseOrder.bindDxDdl("SCODE", data.data.table, selectedValue, "partY_CODE", "partY_NAME", "Select", function (selected) {
    ...
    AddSodaToDelivery: function () {
        ...
        if ($('#SodaPickDetailGridContainer').dxDataGrid('instance').hasEditData()) {
            ...
            if (IsDataAvailableInGrid || empr_PurchaseOrder.firstClick == 1) {
                ...
                var finalData = existingData.concat(selectedSodas);
                empr_PurchaseOrder.pickIds = finalData.map(x => x.picK_ID_D);
                $('#DetailContainer').dxDataGrid('instance').option('dataSource', finalData);
                ...
                if (filteredData.length > 0)
                    $('#PARTY_CODE').dxSelectBox('instance').option('value', filteredData[0].key);
                empr_PurchaseOrder.InitSalesman(0, parseInt(selectedSodas[0].spartY_CODE));
                //empr_PurchaseOrder.InitSalesman(0, parseInt(selectedSodas[0].spartY_CODE));
                //$('#SCODE').dxSelectBox('instance').option('value', parseInt(selectedSodas[0].sacT_CODE));

                empr_PurchaseOrder.InitCurrencyDDL(selectedSodas[0].curR_CODE);
                ...
                $("#DC_NO").val(selectedSodas[0].dC_NO);
                ...
                //$("#RINV_DATE").val(selectedSodas[0].rinV_DATE);
                ...
                empr_PurchaseOrder.pickIds = selectedSodas.map(x => x.picK_ID_D);

                // A.BTYPE, A.DISC AS DISC_M, A.LOCAL_CHARGES, A.DOC, A.COMM_VAL, A.RINV_NO, A.RINV_DATE,

                //selectedSodas.forEach(function (record) {
                //    let payTerm = record.terms;           
                //    let delDate = record.deL_DATE;
                ...
                //    let deliveryDate = new Date(delDate);
                //    deliveryDate.setHours(0, 0, 0, 0);

                //    let result = empr_PurchaseOrder.calculateDue(deliveryDate, payTerm);

                //    if (result && result.isValid) {
                //        record.duE_DATE = result.dueDate;
                //        record.duE_DAYS = result.dueDays;
                //    } else {
                //        record.duE_DATE = null;
                //        record.duE_DAYS = 0;
                //    }
